...
function launch_docker_registry() {
  ...
  local reg_name=${LOCAL_REGISTRY_NAME}	# test-register
  local reg_port=${LOCAL_REGISTRY_PORT}	# 5000
  local reg_interface=${LOCAL_REGISTRY_INTERFACE}	# 127.0.0.1

  running="$(docker inspect -f '{{.State.Running}}' "${reg_name}" 2>/dev/null || true)"	# test-register
  ...
  docker network connect "test" "${reg_name}" || true
