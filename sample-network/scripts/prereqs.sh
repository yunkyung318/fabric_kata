...
function check_prereqs() {
  ...
  if [[ $? -ne 0 ]]; then
    ...
    (pushd $TEMP_DIR && sudo curl -sSL https://raw.githubusercontent.com/hyperledger/fabric/main/scripts/bootstrap.sh \
      | sudo bash -s -- $FABRIC_VERSION $FABRIC_CA_VERSION -s -d)
    sudo mkdir -p bin && sudo mv $TEMP_DIR/bin/* bin

    # delete config files transferred by the installer
    sudo rm $TEMP_DIR/config/configtx.yaml
    sudo rm $TEMP_DIR/config/core.yaml
    sudo rm $TEMP_DIR/config/orderer.yaml
  fi
